...
```{r, out.width="100%"}
ggplotly( ggplot(combined_DF %>% group_by(league, year) %>% summarise(no_matches = length(year), .groups = 'drop'),
...
ggplotly( ggplot(combined_DF %>% filter(tourney_name %in% top_tourney$tourney_name) %>% group_by(tourney_name) %>% count(year)) + 
...
```{r}
ggplotly( ggplot(combined_DF %>% filter(!is.na(winner_hand) & !is.na(loser_hand)) %>%
         mutate(winner_hand=recode(winner_hand, L="Left Hand", R="Right hand", U="Both Hands")) %>%
...
Interactive page with ranking evolution of players
...
server <- function(input, output, session) {
  ...
  uiRankPlayers <- reactive({
    ...
    max_rank <- players_DF %>% mutate(birth_date=as.Date(birth_date), full_name=paste0(last_name, " ", first_name)) %>%
      filter(birth_date > as.Date('1990-01-01')) %>%
...
server <- function(input, output) {
  ...
  plot_confusion <- function(model, observations) {
    ...
    ggplot(data = confusion_values, mapping = aes(x = Prediction, y = Reference)) +
  ...
  output$knn_confusion <- renderPlot({plot_confusion(knn_model, observations_test)})
  output$knn_shapley <- renderPlot({plot_shapley(knn_model, observations_train, observations_test)})
  output$knn_perf <- renderPlot({plot_performance_knn(knn_model)})
  
  output$rf_confusion <- renderPlot({plot_confusion(rf_model, observations_test)})
  output$rf_shapley <- renderPlot({plot_shapley(rf_model, observations_train, observations_test)})
  output$rf_perf <- renderPlot({plot_performance_rf(rf_model)})

  output$svm_confusion <- renderPlot({plot_confusion(svm_model, observations_test)})
  output$svm_shapley <- renderPlot({plot_shapley(svm_model, observations_train, observations_test)})
  output$svm_perf <- renderPlot({plot_performance_svm(svm_model)})

  output$lgreg_confusion <- renderPlot({plot_confusion(lgreg_model, observations_test)})
  output$lgreg_shapley <- renderPlot({plot_shapley(lgreg_model, observations_train, observations_test)})
  # output$lgreg_perf <- renderPlot({plot_performance_knn(lgreg_model)})

  output$xgb_confusion <- renderPlot({plot_confusion(xgb_model, observations_test)})
  output$xgb_shapley <- renderPlot({plot_shapley(xgb_model, observations_train, observations_test)})
  output$xgb_perf <- renderPlot({plot_performance_xgb(xgb_model)})
}
