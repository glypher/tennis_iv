---
title: "ATP and WTA Tennis Matches"
author: "Mihai Matei, Catalin Stefan Cernat - 511 DS"
date: "3/24/2021"
output:
  html_document:
    df_print: paged
...
### Project aim
The aim of our project is two-fold. First, we want to do Exploratory Data Analysis on all the available data
to try to find insightful and interesting statistics of any kind. Secondly, we would like to apply what we've
learned from exploring the data in order to train regression / classification models for different tasks.

### Packages
This project uses a wide range of packages, from indispensable ones like `rmarkdown` and `tidyverse` used in every chunk,
to some which are more use case specific like `shiny` and `shinyWidgets` for interactive dashboards,
`caret`, `xgboost`, `shapr` and `iml` for model training and interpretation and many more.
...
load_packages(c('rmarkdown', 'tidyverse', 'lubridate', 'reshape2', 'viridis', 'maps', 'RJSONIO', "shiny", "shinydashboard", "caret", "kernlab", "e1071", "xgboost", "plotly", "shapr", "arm", "shinyWidgets", "iml", "gridExtra", "ggnetwork", "network"))
slice <- dplyr::slice
select <- dplyr::select
```
...
#### ATP and WTA matches csv files
Load the 128 `.csv` files containing matches for the ATP and WTA leagues. For each file,
parse all variables with their correct data type, set correct ordered factors levels,
create synthetic features. Finally, merge the data from the two leagues and create
a new discriminator variable.

Each data point contains 32 features, 2 continuous and 30 discrete.
...
```{r load_matches, message=FALSE, warning=FALSE}
...
#### ATP and WTA player csv files
Load the two `.csv` files containing information for all ATP (55,022) and WTA (35,158) players like
name, country and date of birth. Same as before, we combine the data from the two leagues
and create a new discriminator variable.

```{r load_players, message=FALSE, warning=FALSE}
col_names <- c(
...
#### ATP and WTA rankings csv files
Aside from matches and players info, the repository also contains ranking data. For both ATP and WTA, weekly standings are available
starting from the 70s and 80s respectively. In total 4,664,522 data points are available.

```{r load_rankings, message=FALSE, warning=FALSE}
...
Because for each player we only had the IOC code of his home country and we wanted the full name of the country,
we obtained another dataset linking the two.

```{r load_countries, message=FALSE, warning=FALSE}
column_types <- cols(
  country = "c",
  ioc = "c"
)
...
Evolution of the number of matches in the sport of tennis in recent years.
In the 1970s there were many tennis tournaments such as WCT and NLT which competed for male players.
At the end of 1972 US Open due to increased rivalry between different tournaments players agreed to form the Association of Tennis Professionals (ATP)
and to boycott several important tournaments such as Wimbledon.
In 2020 the impact of Coronavirus pandemic was a severe blow for the sport.
...
The location on the world map of the top 50 tournaments.
We get the latitude and longitude coordinates by tournament name using openstreetmap online API.
The vast majority of tournaments are held in Europe, North America, and Australia / New Zealand.
Only 5 tournaments are held outside these locations, Africa and South America only having one each.
```{r plot_tourney_location, out.width="100%", class.source='bigPlot', message=FALSE, warning=FALSE}
...
renderPlotly({
  ggplotly(ggplot(map_data("world"), aes(x=long, y=lat)) +
             ...
             ggtitle("Tourney world map") +
             theme_classic() +
             theme(legend.position = "none") +
             coord_quickmap(), height = 600)
  })
```

World Map showing how many tournaments (ATP and WTA) individual countries have won.
In the previous plot we saw that most tournaments are held in Europe, North America or Australia / New Zealand.
Here, we can see almost the same trend present. Europe leads with most tourney wins, followed by the U.S. and Australia.
However, except for Africa and S.E.A., there aren't many regions with a lack of tournament wins.
...
wins_per_country_plot <- function() {
  ...
  return (
    ggplotly(static_plot, tooltip = c('label_country', 'label_combined', 'label_atp', 'label_wta'), height = 600)
    )
}
...
Evolution of matches per year for the top 6 most played tournaments.
While the data doesn't go back as far as the beginning of these tournaments we can still see some interesting things.
For example, there is a sharp increase in the Australian Open starting in the second half of the 80s. This is due to the tournament being
moved Melbourne Park thus being more accessible and attracting more top level players.

Another interesting data points can be seen in 1968, specifically by looking at Wimbledon. This was the year when
amateur players (which played in Grand Slams but received no remuneration) were allowed to play with professionals
(which didn't play in Grand Slams but received remuneration from the nobility). This is called the Open Era of tennis.
...
### Match statistics
Breakdown of marches by surface type. The most common surfaces types, for both ATP and WTA, are clay surfaces and hard surfaces.
This is due to the fact that these surfaces are easier to maintain than grass and offer consistency
(they don't deteriorate as fast grass does and if they do they can more easily be repaired, as opposed to waiting for the grass to grow again).
...
Average match length by round type. Most round types have a mean match duration of around 90 minutes and about the same
variance. The notable exception here would be R128 for ATP (maybe the players are more rested?). ATP matches also last longer
than WTA ones (men might have more stamina; Grand Slams are also best of 5 for men).
...
Correlation matrix between various serve statistics and match wins.
The number of breaks have the strongest correlation with a match win. To obtain a break, a player
needs to win a set in which the opponent is the one serving. Interestingly there is little difference here
between men and women (0.57 vs 0.59)

One notable difference between men and women is the correlation between getting the first serve inside the court
and the number of aces. It is much higher for men (0.35) than it is for women (0.13).
```{r plot_correlation, out.width="100%", message=FALSE, warning=FALSE}

corr_DF <- combined_DF %>%
  select(league, w_1stIn, w_1stWon, w_2ndWon, w_breaks, w_ace, w_df) %>%
  filter(!is.na(w_1stIn), !is.na(w_1stWon), !is.na(w_2ndWon), !is.na(w_breaks), !is.na(w_ace), !is.na(w_df)) %>%
  rename(fstIn = w_1stIn, fstWon = w_1stWon, sndWon = w_2ndWon, breaks = w_breaks, aces = w_ace, dFaults = w_df) %>% mutate(win = 1) %>%
  bind_rows(combined_DF %>%
              select(league, l_1stIn, l_1stWon, l_2ndWon, l_breaks, l_ace, l_df) %>%
              filter(!is.na(l_1stIn), !is.na(l_1stWon), !is.na(l_2ndWon), !is.na(l_breaks), !is.na(l_ace), !is.na(l_df)) %>%
              rename(fstIn = l_1stIn, fstWon = l_1stWon, sndWon = l_2ndWon, breaks = l_breaks, aces = l_ace, dFaults = l_df) %>% mutate(win = 0))

atp_serves_correlation <- cor(corr_DF %>% filter(league == "ATP") %>% select(!league))
wta_serves_correlation <- cor(corr_DF %>% filter(league == "WTA") %>% select(!league))
combined_server_correlation <- bind_rows(list(ATP = melt(atp_serves_correlation), WTA = melt(wta_serves_correlation)), .id = "league")

ggplotly(
  ggplot(data = combined_server_correlation, aes(x = Var1, y = Var2, fill = value)) +
    geom_tile() +
    facet_grid(cols=vars(league)) +
    theme_classic() +
    scale_fill_viridis_c() +
    labs(x="Variable 1", y="Variable 2", title="Correlation Matrix for serve statistics"))
```

Percentage of serves won if the first serve lands inside the court. In tennis, for each game, a player gets
a maximum of two serves. If the first one lands outside the court area or hits the net, the player is allowed
another try. Because of this, players try to make their first server as good as possible and in turn making them more risky
(e.g. they try to land the ball as close as possible to the lines). If the serve lands, there is high probability that
the player will win the server, ~71% for ATP and ~63% for WTA.
...
Visualize the mean age by rank (averaged over an year) across the years.
We can see that throughout the years the mean age over the same rank is higher for men
than women.

In 1985 the average are for rank 1 ATP and WTA was 25 years and 29 years respectively.
While there were the occasional rank 1 over 30 (with note in WTA), most of the top tennis
players were under 30. We can observe a slow an steady increase in age for top tennis players
over the years. In the last years the top WTA player was frequently over 30 and the current ATP rank 1
is over 30.
...
age_by_rank_plot <- function() {
  ...
  static_plot <- ggplot(df, aes(rank, avg_age, frame = year)) +
    ...
    scale_color_manual(values=c('#3366ff','red')) +
    labs(x="Rank", y="Mean age", title="Mean age by rank")
...
Number of active players by year for ATP and WTA. We saw in the last graph that as we progressed
in time, there were more and more rank positions. This is due to the increase of active players in the ATP and
WTA tournaments.

We see a sharp increase starting in 1983 as ATP started to consolidate due to other
tours starting to lose ground (Grand Prix and WCT - World Championship Tennis).
There is also a decrease starting with 2015. This is mostly due to inactive players
passively losing all their ATP points and no longer being included in the rankings.
...
build_players_by_year_plot <- function() {
  ...
  static_plot <- ggplot(df,aes(x=ranking_year, y=n_players, group=league)) +
    ...
    geom_line(aes(color=league), size=1) +
    scale_color_manual(values=c('#3366ff','red')) +
    labs(x="Year", y="Number of players", title="Number of active players per year") +
    theme_classic()
...
Distribution of rank difference of players in matches. Notice it is Gaussian with 0 mean.
For better visualization we also zoom in for the ranks difference between -150 and 150.
This suggests that most matches are balanced. This is achieved in multiple ways.
Firstly, tournaments usually don't have big discrepancies between players due to
entry requirements. Moreover, the seeds are carefully chosen not to create
unbalanced matches. Secondly, as a tournament progresses, players naturally
face other players with a similar skill level.
  ...
  sidebarLayout(
    ...
    mainPanel(
      tabsetPanel(type = "tabs",
                  tabPanel("Ranks", fluidRow(
                    plotOutput("rankPlot"),
                    tableOutput("playerTable"))),
                  tabPanel("Statistics", fluidRow(
                    plotOutput("acePlot", height=250),
...
server <- function(input, output, session) {
  ...
  observeEvent(uiPlayers(), {
    ...
    stat_DF <- combined_DF %>% select(winner_id, w_1stWon, w_2ndWon, w_breaks) %>%
      rename(player_id=winner_id) %>% inner_join(players_DF, by=c("player_id")) %>%
    ...
    games_DF <- combined_DF %>% select(winner_id, loser_id) %>%
      inner_join(players_DF, by=c("loser_id"="player_id")) %>% rename(loser_name=full_name) %>%
      inner_join(players_DF, by=c("winner_id"="player_id")) %>% rename(winner_name=full_name) %>%
      group_by(winner_id, winner_name, loser_id, loser_name) %>%
      summarise(n=length(loser_id)) %>% ungroup() %>%
      mutate(winner_id=as.integer(as.factor(winner_id), levels=unique(winner_id)),
             loser_id=as.integer(as.factor(loser_id), levels=unique(winner_id)))
    if (nrow(games_DF) == 0) return()

    graph<-network.initialize(max(games_DF$winner_id), loops=TRUE, directed=TRUE)
    network.vertex.names(graph) <- unique(games_DF$winner_name)
    add.edges(graph, games_DF$winner_id, games_DF$loser_id)
    set.edge.attribute(graph, "games", games_DF$n)

    output$graphPlot <- renderPlot({
      ggplot(ggnetwork(graph), aes(x = x, y = y, xend = xend, yend = yend)) +
        geom_nodetext(aes(color=vertex.names, label=vertex.names), fontface="bold") +
        geom_edgetext_repel(aes(label=games), color = "white", fill = "tomato", box.padding = unit(1, "lines")) +
        geom_edges(color = "black", curvature=0.2, angle=30, size=1, arrow=arrow(length = unit(10, "pt"), type = "closed")) +
        scale_color_viridis_d() + labs(title="Graph of players win/loose history") + theme_classic()
    })
  })
...
### Classification models for predicting match wins
